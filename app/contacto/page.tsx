...
export default function ContactPage() {
  ...
  const validateForm = () => {
    ...
    if (!formData.message.trim()) {
      newErrors.message = "Por favor contanos sobre tu proyecto"
      isValid = false
    }
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const whatsappMessage = `¡Hola! Soy *${formData.name}*%0A` +
          ...
          <motion.div initial="hidden" animate="visible" variants={staggerContainer} className="max-w-4xl mx-auto">
            <motion.div variants={fadeIn} className="text-center mb-12">
              ...
              <p className="text-gray-300 text-lg mt-6 max-w-2xl mx-auto">
                Completá el formulario y tu mensaje se enviará directamente por WhatsApp. 
                    ...
                    <div className="text-right">
                      ...
                      >
                        <span className="absolute inset-0 flex items-center justify-center w-full h-full text-white duration-300 -translate-x-full bg-gradient-to-r from-purple-600 to-indigo-600 group-hover:translate-x-0 ease">
                          {isRedirecting ? (
                            <svg className="w-6 h-6 mr-1 animate-spin" fill="none" viewBox="0 0 24 24">
                        ...
                        <span className="absolute flex items-center justify-center w-full h-full text-white transition-all duration-300 transform group-hover:translate-x-full ease">
                          {isRedirecting ? "Abriendo WhatsApp..." : "Enviar por WhatsApp"}
                        </span>
                        <span className="relative invisible">{isRedirecting ? "Abriendo WhatsApp..." : "Enviar por WhatsApp"}</span>
                      </motion.button>
                    </div>
                  </form>
                </div>
